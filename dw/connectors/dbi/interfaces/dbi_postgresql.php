...
class dbi_postgresql extends dbiInterface {
	...
	public function setClientEncoding($encoding) {
		return pg_set_client_encoding($this -> _reslink, $encoding);
	}
	...
	public function getSchemaTable($stablename) {
		$ary = null;
		$res = $this->query ( "SELECT relfilenode, attnum, attname FROM pg_class, pg_attribute 
		...
		if ($res) {
			...
			$res = $this->query ( "SELECT * FROM pg_index 
				WHERE indrelid = " . $id . " 
				AND indisprimary = 't'" );
			if ($res) {
